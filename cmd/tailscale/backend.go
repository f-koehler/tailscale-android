...
var googleDnsServers = []netaddr.IP{netaddr.IPv4(8, 8, 8, 8), netaddr.IPv4(8, 8, 4, 4),
...
func (b *backend) logDnsConfigMechanisms() {
	...
	err := jni.Do(b.jvm, func(env *jni.Env) error {
		...
		m := jni.GetMethodID(env, cls, "getDnsConfigFromLinkProperties", "()Ljava/lang/String;")
		n, err := jni.CallObjectMethod(env, b.appCtx, m)
		baseConfig = jni.GoString(env, jni.String(n))
		return err
	})
	if err != nil {
		log.Printf("getDnsConfigFromLinkProperties JNI: %v", err)
	} else {
		log.Printf("getDnsConfigFromLinkProperties: %s", baseConfig)
	}

	err = jni.Do(b.jvm, func(env *jni.Env) error {
		cls := jni.GetObjectClass(env, b.appCtx)
		m := jni.GetMethodID(env, cls, "getDnsServersFromSystemProperties", "()Ljava/lang/String;")
		n, err := jni.CallObjectMethod(env, b.appCtx, m)
		baseConfig = jni.GoString(env, jni.String(n))
		return err
	})
	if err != nil {
		log.Printf("getDnsServersFromSystemProperties JNI: %v", err)
	} else {
		log.Printf("getDnsServersFromSystemProperties: %s", baseConfig)
	}

	err = jni.Do(b.jvm, func(env *jni.Env) error {
		cls := jni.GetObjectClass(env, b.appCtx)
		m := jni.GetMethodID(env, cls, "getDnsServersFromNetworkInfo", "()Ljava/lang/String;")
		n, err := jni.CallObjectMethod(env, b.appCtx, m)
		baseConfig = jni.GoString(env, jni.String(n))
		return err
	})
	if err != nil {
		log.Printf("getDnsServersFromNetworkInfo JNI: %v", err)
	} else {
		log.Printf("getDnsServersFromNetworkInfo: %s", baseConfig)
	}
}
